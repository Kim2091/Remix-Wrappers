 ...
 * ============================================================
 *
 * OR2006 does NOT upload separate View or Projection matrices to VS
 * constants. The rendering pipeline is:
 *
 *   - 0x411060 (per-draw):  Computes WorldView = World * View,
 *                            uploads Transpose(WorldViewProj) to VS reg 0x40
 *   - 0x95DB20 (global):    WorldView = World * View, updated per draw
 *   - 0x95D860 (global):    Camera "view" = [Identity | cam_pos] (no rotation)
 *   - 0x95D8A0 (global):    Projection matrix (D3DXMatrixPerspectiveFovRH output)
 *
 * To reconstruct per-object World: World = WorldView - cam_pos in row 3
 *   (exact because View has identity rotation, so World.upper3x3 = WorldView.upper3x3)
 *
 * VS_REG_WORLD_START/END covers reg 0x40 (64) to trigger worldDirty on
 * each per-draw WorldViewProj upload. VIEW/PROJ are read from game globals.
 * ============================================================ */
#define VS_REG_VIEW_START       0   /* unused: view read from game global 0x95D860 */
#define VS_REG_VIEW_END         4
#define VS_REG_PROJ_START       4   /* unused: proj read from game global 0x95D8A0 */
#define VS_REG_PROJ_END         8
#define VS_REG_WORLD_START     64   /* reg 0x40: Transpose(WorldViewProj) per draw */
#define VS_REG_WORLD_END       68

/* Bone detection: disabled for OR2006 (per-object extra VS writes are single
 * matrices with count=4, well below the BONE_MIN_REGS=9 threshold). */
#define VS_REG_BONE_THRESHOLD  80   /* regs 0x50+ are per-object supplemental data */
#define VS_REGS_PER_BONE        3
#define VS_BONE_MIN_REGS        9
...
typedef struct WrappedDevice {
    ...
    int curDeclHasColor;
    int curDeclHasTangent;   /* normal-mapped surface (water): needs FFP-compat decl swap */
    int curDeclNumElems;
    unsigned char curDeclElems[8 * 20];

    /* Cached FFP-compatible declaration for TANGENT surfaces (water).
     * D3D9 FFP cannot use declarations containing TANGENT elements.
     * This is a trimmed POS/NORMAL/TEX0-only decl with the same vertex
     * offsets as the real water declaration, allowing FFP to read the
     * correct fields while ignoring the unsupported TANGENT/TEX1. */
    void *waterFFPDecl;
    ...
    int waterDiagDone;  /* 1 after first TANGENT-decl DIP is logged */
} WrappedDevice;
...
/* 4x4 row-major matrix multiply: dst = a * b */
static void mat4_mul(float *dst, const float *a, const float *b) {
    int r, c;
    for (r = 0; r < 4; r++) {
        for (c = 0; c < 4; c++) {
            dst[r*4+c] = a[r*4+0]*b[c]
                       + a[r*4+1]*b[4+c]
                       + a[r*4+2]*b[8+c]
                       + a[r*4+3]*b[12+c];
        }
...
/*
 * Compute the inverse of a D3DX LookAt view matrix (D3D row-major).
 *
 * A LookAt matrix has an orthonormal 3x3 upper block, so:
 *   inv(R) = R^T
 *   eye = -R^T * translation_row
 *
 * The inverse is the camera-to-world (view-to-world) matrix, with the
 * camera's world-space position in row3.
 */
static void view_inverse(float *dst, const float *v) {
    /* Transpose the upper 3x3 */
    dst[0]  = v[0];  dst[1]  = v[4];  dst[2]  = v[8];  dst[3]  = 0.0f;
    dst[4]  = v[1];  dst[5]  = v[5];  dst[6]  = v[9];  dst[7]  = 0.0f;
    dst[8]  = v[2];  dst[9]  = v[6];  dst[10] = v[10]; dst[11] = 0.0f;
    /* Recover camera world position: eye = -R^T * t */
    dst[12] = -(v[0]*v[12] + v[1]*v[13] + v[2]*v[14]);
    dst[13] = -(v[4]*v[12] + v[5]*v[13] + v[6]*v[14]);
    dst[14] = -(v[8]*v[12] + v[9]*v[13] + v[10]*v[14]);
    dst[15] = 1.0f;
}
 ...
 */
/*
 * OR2006-specific game memory pointers.
 *
 * 0x95D860: Per-frame camera matrix = [Identity | cam_pos]. Updated by
 *           D3DXMatrixLookAtRH in fcn.00422AC0. Upper 3x3 is identity
 *           (no camera rotation), row 3 = [cam_x, cam_y, cam_z, 1].
 *
 * 0x95D8A0: Projection matrix. Set once per scene by D3DXMatrixPerspectiveFovRH
 *           (fovY~0.52 rad, aspect 2.0, zn=0.1, zf=5000). Standard RH perspective.
 *
 * 0x95DB20: Per-draw WorldView = World * View (0x95D860). Computed in
 *           fcn.00411060 immediately before each DrawIndexedPrimitive.
 *           Valid to read at DIP time for the current object.
 */
#define OR2006_VIEW_GLOBAL      ((float *)0x0095D860)
#define OR2006_PROJ_GLOBAL      ((float *)0x0095D8A0)
#define OR2006_WORLDVIEW_GLOBAL ((float *)0x0095DB20)
...
static void FFP_ApplyTransforms(WrappedDevice *self) {
    ...
    /*
     * View: submit the live LookAt matrix directly.
     * OR2006's camera at 0x95D860 is a full D3DXMatrixLookAtRH output
     * with rotation — NOT an identity-rotation matrix as previously assumed.
     */
    ((FN_SetTransform)vt[SLOT_SetTransform])(
        self->pReal, D3DTS_VIEW, OR2006_VIEW_GLOBAL);

    /*
     * Projection: read from live game global.
     * Set once per scene via D3DXMatrixPerspectiveFovRH.
     */
    ((FN_SetTransform)vt[SLOT_SetTransform])(
        self->pReal, D3DTS_PROJECTION, OR2006_PROJ_GLOBAL);

    if (self->worldDirty) {
        /*
         * World = WorldView * View^(-1)
         *
         * OR2006 only exposes WorldView (= World * View) per draw at 0x95DB20.
         * View has full rotation, so we cannot recover World by simply
         * subtracting cam_pos. The correct recovery is:
         *
         *   World = WorldView * inv(View)
         *
         * For an orthonormal view matrix, inv(View) is cheap:
         *   - upper 3x3: transpose
         *   - translation row: -R^T * t  (camera world position)
         *
         * Submitting this as D3DTS_WORLD lets RTX Remix correctly separate
         * the camera from per-object world transforms for ray tracing.
         */
        view_inverse(viewInv, OR2006_VIEW_GLOBAL);
        mat4_mul(worldMat, OR2006_WORLDVIEW_GLOBAL, viewInv);

        ((FN_SetTransform)vt[SLOT_SetTransform])(
            self->pReal, D3DTS_WORLD, worldMat);
...
/* ---- Water FFP declaration helper ---- */

/* Returns a cached FFP-compatible vertex declaration for water draws.
 * Water uses TANGENT elements which D3D9 FFP refuses — this decl strips them,
 * keeping the same field offsets so FFP reads POS/NORMAL/TEX0 correctly. */
static void *WaterFFPDecl_Get(WrappedDevice *self)
{
    /* D3DVERTEXELEMENT9: stream(WORD), offset(WORD), type(BYTE), method(BYTE),
     *                    usage(BYTE), usageIndex(BYTE) = 8 bytes each
     * Types:  FLOAT3=2, FLOAT2=1
     * Usages: POSITION=0, NORMAL=3, TEXCOORD=5
     * D3DDECL_END sentinel: stream=0xFF, type=0x11 */
    static const unsigned char kWaterFFPDecl[] = {
        0x00,0x00, 0x00,0x00, 0x02, 0x00, 0x00, 0x00,  /* POS   FLOAT3 offset  0 */
        0x00,0x00, 0x0C,0x00, 0x02, 0x00, 0x03, 0x00,  /* NRM   FLOAT3 offset 12 */
        0x00,0x00, 0x18,0x00, 0x01, 0x00, 0x05, 0x00,  /* TEX0  FLOAT2 offset 24 */
        0xFF,0x00, 0x00,0x00, 0x11, 0x00, 0x00, 0x00   /* END */
    };

    if (!self->waterFFPDecl) {
        typedef int (__stdcall *FN_CreateDecl)(void*, const void*, void**);
        void *decl = NULL;
        int hr = ((FN_CreateDecl)RealVtbl(self)[SLOT_CreateVertexDeclaration])(
            self->pReal, kWaterFFPDecl, &decl);
        if (hr == 0 && decl) {
            self->waterFFPDecl = decl;
            log_str("WaterFFPDecl_Get: FFP-compat water decl created\r\n");
        } else {
            log_hex("WaterFFPDecl_Get: CreateVertexDeclaration failed hr=", (unsigned int)hr);
        }
    }
    return self->waterFFPDecl;
}
...
static int __stdcall WD_DrawPrimitive(WrappedDevice *self, unsigned int pt, unsigned int sv, unsigned int pc) {
    ...
    if (DIAG_ACTIVE(self) && self->drawCallCount <= 200) {
        ...
        if (self->drawCallCount <= 5) {
            diag_log_matrix("    or2006_view(glob)", OR2006_VIEW_GLOBAL);
            diag_log_matrix("    or2006_proj(glob)", OR2006_PROJ_GLOBAL);
            diag_log_matrix("    or2006_worldview",  OR2006_WORLDVIEW_GLOBAL);
        }
    }
#endif
    return hr;
    ...
    unsigned int si, unsigned int pc)
{
    ...
    if (self->viewProjValid) {
        if (self->curDeclIsSkinned) {
            ...
            /*
             * Rigid mesh (including water/tangent surfaces): FFP conversion.
             *
             * Water uses a TANGENT vertex declaration and a VS that animates
             * wave displacement. In FFP mode the mesh renders as a static flat
             * plane at its authored sea-level position — no animation, but
             * Remix sees it as FFP geometry and can apply a flipbook water
             * material on top. Stage 1 (normal map) is already disabled by
             * FFP_SetupTextureStages; stage 0 holds the base water colour.
             */

#if DIAG_ENABLED
            /* Water-specific diagnostic: dump all VS constants at first TANGENT draw.
             * Fires after 5s regardless of main DIAG_ACTIVE window. */
            if (self->curDeclHasTangent && !self->waterDiagDone &&
                GetTickCount() - self->createTick >= 5000)
            {
                int r;
                self->waterDiagDone = 1;
                log_str("=== WATER DIP DIAG (TANGENT decl) ===\r\n");
                log_int("  DIP drawCall#", self->drawCallCount);
                log_int("  numVerts=", nv);
                log_int("  primCount=", pc);
                log_str("  VS consts written this frame:\r\n");
                for (r = 0; r < 256; r++) {
                    if (self->vsConstWriteLog[r]) {
                        float *c = &self->vsConst[r * 4];
                        log_int("    c", r);
                        log_floats_dec("      ", c, 4);
                    }
                }
            ...
            /* For water (TANGENT decl): swap in an FFP-compatible declaration.
             * D3D9 FFP refuses to render with TANGENT/BINORMAL elements present.
             * We use a cached decl with the same offsets but only POS/NORMAL/TEX0,
             * so FFP reads the correct vertex fields from the same vertex buffer. */
            if (self->curDeclHasTangent) {
                typedef int (__stdcall *FN_SetDecl)(void*, void*);
                void *ffpDecl = WaterFFPDecl_Get(self);
                if (ffpDecl) {
                    ((FN_SetDecl)RealVtbl(self)[SLOT_SetVertexDeclaration])(
                        self->pReal, ffpDecl);
                }
            }
            ...
            /* Restore the real vertex declaration after a water FFP draw. */
            if (self->curDeclHasTangent && self->lastDecl) {
                typedef int (__stdcall *FN_SetDecl)(void*, void*);
                ((FN_SetDecl)RealVtbl(self)[SLOT_SetVertexDeclaration])(
                    self->pReal, self->lastDecl);
            }
            {
                typedef int (__stdcall *FN_SetTex)(void*, unsigned int, void*);
                int as = self->albedoStage;
    ...
    if (DIAG_ACTIVE(self) && self->drawCallCount <= 200) {
        ...
        if (self->drawCallCount <= 5) {
            /*
             * OR2006: log actual game globals, not vsConst[].
             * View/Proj are read from fixed global addresses.
             * World is reconstructed from WorldView - cam_pos.
             */
            {
                float viewInvD[16];
                float worldMatD[16];
                view_inverse(viewInvD, OR2006_VIEW_GLOBAL);
                mat4_mul(worldMatD, OR2006_WORLDVIEW_GLOBAL, viewInvD);
                diag_log_matrix("    or2006_view(glob)",   OR2006_VIEW_GLOBAL);
                diag_log_matrix("    or2006_proj(glob)",   OR2006_PROJ_GLOBAL);
                diag_log_matrix("    or2006_worldview",    OR2006_WORLDVIEW_GLOBAL);
                diag_log_matrix("    computed_world",      worldMatD);
            }
        }
    }
#endif
        ...
        if (hr2 == 0 && numElems > 0 && numElems <= 32) {
            ...
            if (hr2 == 0) {
                ...
                for (e = 0; e < numElems; e++) {
                    ...
                    if (usage == D3DDECLUSAGE_TANGENT) {
                        self->curDeclHasTangent = 1;
                    }
                }
...
WrappedDevice* WrappedDevice_Create(void *pRealDevice) {
    ...
    w->lastPS = NULL;
    /*
     * OR2006: View and Proj are read from fixed game globals (0x95D860 / 0x95D8A0),
     * not from VS constant registers. They are always valid before the first draw,
     * so mark viewProjValid immediately rather than waiting for register writes
     * that will never come.
     */
    w->viewProjValid = 1;
    w->lastDecl = NULL;
    w->curDeclIsSkinned = 0;
    w->curDeclHasTangent = 0;
    w->waterFFPDecl = NULL;
#if ENABLE_SKINNING
