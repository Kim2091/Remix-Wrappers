...
namespace comp
{
	...
	{
		// Reopen the log file from scratch
		if (log_file_ != INVALID_HANDLE_VALUE)
		{
			CloseHandle(log_file_);
			log_file_ = INVALID_HANDLE_VALUE;
		...
		if (log_file_ != INVALID_HANDLE_VALUE)
		{
			log_str("FFP Proxy Diagnostic Log (manual trigger via F11)\r\n");
			log_str("==================================================\r\n");
			log_int("Log frames: ", max_frames_);
			log_str("\r\n");
		}
		...
		for (auto& d : logged_decls_) d = nullptr;
		for (auto& u : diag_tex_uniq_) u = 0;

		// Make is_active() fire on the very next frame regardless of delay_ms_
		create_tick_ = GetTickCount() - delay_ms_ - 1;

		shared::common::log("Diagnostics", "Manual trigger: capturing next 3 frames");
	}
	...
	{
		if (!is_active() || draw_count > 2000) return;
	...
	{
		// F11 manual trigger — edge-detected so a held key doesn't retrigger
		{
			static bool was_pressed = false;
			bool pressed = (GetAsyncKeyState(VK_F11) & 0x8000) != 0;
			if (pressed && !was_pressed) restart();
			was_pressed = pressed;
		}
	...
	void diagnostics::route(const char* tag)
	{
		if (!is_active() || !tag) return;
		log_str("    ROUTE: ");
		log_str(tag);
		log_str("\r\n");
	}
